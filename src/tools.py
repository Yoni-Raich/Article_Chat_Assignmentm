...
def search_articles(query: str, max_results: int = 5) -> List[Dict]:
    ...
    for search_result in search_results:
        simplified.append({
            ...
            "relevance": search_result["similarity_score"]
        })
...
@tool
def find_most_similar_article(input_text: str, similarity_threshold: float = 0.5) -> Optional[Dict]:
    """
    Find the most similar article to given text. use this when the user only describes the article.

    Args:
        input_text: Text to match against
    ...
    search_results_similar = VECTOR_STORE.search(input_text, k=1)

    if not search_results_similar:
        return None

    best_match = search_results_similar[0]
...
def get_most_common_entities(
    ...
    Args:
        entity_type: Type of entities to filter by
                    ("all", "people", "organizations", "locations", "products")
    ...
    for article in articles:
        ...
        for entity in entities:
            ...
            if entity_type != "all":
                ...
                if (entity_type == "people" and
                    not any(word in entity_lower for word in
                           ["corp", "inc", "ltd", "company", "tech", "city", "country", "state"])):
                    pass  # Likely a person
                elif (entity_type == "organizations" and
                      any(word in entity_lower for word in
                          ["corp", "inc", "ltd", "company", "tech"])):
                    pass  # Likely an organization
                elif (entity_type == "locations" and
                      any(word in entity_lower for word in
                          ["city", "country", "state", "county"])):
                    pass  # Likely a location
                elif (entity_type == "products" and
                      not any(word in entity_lower for word in
                              ["corp", "inc", "ltd", "city", "country", "state"])):
...
def get_all_articles(field: str = "title") -> List[str]:
    ...
    When user asks to "list all articles" or similar, use this tool without 
    specifying the field parameter (it will default to titles).
